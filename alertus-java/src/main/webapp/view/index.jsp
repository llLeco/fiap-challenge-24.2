...
<%@ page import="br.com.fiap.alertus.model.Notification" %>
                            ...
                            <% 
                                ...
                                if (notifications != null) {
                                    for(Notification n : notifications) {
                                        if ("High".equals(n.getLevel()) || "Alto".equals(n.getLevel())) criticos++;
                                    }
                                }
                                out.print(criticos);
                            %>
        ...
        <div class="row">
            <div class="col-12">
                <div class="table-container">
                    <h3 class="mb-3">
                        <i class="bi bi-bell"></i> Alertas Ativos (<%=totalNotifications%>)
                    </h3>
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <% for(Notification n : notifications) { %>
                                        <tr>
                                            <td>
                                                <span class="badge bg-primary"><%=n.getEvent().getType()%></span>
                                            </td>
                                            <td><%=n.getEvent().getRegion().getName()%></td>
                                            <td><%=n.getMessage()%></td>
                                            <td>
                                                <span class="status-badge 
                                                    <% if ("High".equals(n.getLevel()) || "Alto".equals(n.getLevel())) { %>status-alta<% } 
                                                       else if ("Medium".equals(n.getLevel()) || "Medio".equals(n.getLevel())) { %>status-moderada<% } 
                                                       else { %>status-baixa<% } %>">
                                                    <%=n.getLevel()%>
                                                </span>
                                            </td>
